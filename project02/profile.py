...
Resources for profiling sort and fibonacci functions as well
as the nQueens functions.
"""
...
from nQueens import nQueensBrute, nQueensFancy
...
def profileQueens(queensFunction, upper = 4):
    """Displays the board size, number of calls, and the
        run time of a given queens function"""
    n = 4
    counter = Counter()
    print(" n        # Call  Running Time (sec)")
    for i in range(upper):
        counter.reset()
        t1 = time.time()
        queensFunction(n, counter)
        t2 = time.time() - t1
        print("%3d%12s%14.6f" % (n, counter, t2))
        n += 1
...
def main():
    ...
    profileFib(fib1)
    profileFib(fib2)
    profileQueens(nQueensBrute)
    profileQueens(nQueensFancy)
